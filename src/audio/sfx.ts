...
export class Sfx {
  ...
  focus(dur: number): void {
    this.eng.tone({ freq: D3, dur: dur + 0.5, attack: dur * 0.6, gain: 0.09, type: 'triangle', wet: 0.7 })
    this.eng.tone({ freq: D3 * 1.5, dur: dur + 0.4, attack: dur * 0.7, gain: 0.045, wet: 0.7 })
  }

  /** Beat A — @ and λ flash as one unit: a warm rising hum. */
  identify(dur: number): void {
    this.eng.tone({ freq: D4, endFreq: this.note(D4, 1), dur: dur + 0.25, attack: 0.05, gain: 0.085, type: 'triangle', wet: 0.6 })
    this.eng.tone({ freq: A4, dur: dur + 0.15, attack: 0.04, gain: 0.04, wet: 0.7 })
  }
  ...
  fly(dur: number, copies: number): void {
    this.eng.wash({ from: 280, to: 1500, dur, gain: 0.09 + 0.02 * Math.min(4, copies), wet: 0.5, q: 1.4 })
  }

  /** A copy lands on its variable: a glass chime in the binder's hue. */
  land(hue: number | null, pan: number): void {
    this.eng.chime({ freq: this.hueFreq(hue), dur: 1.6, gain: 0.085, pan, wet: 0.8 })
  }

  /** Unused argument evaporates: a falling breath, no chime — nothing
   *  arrives anywhere. */
  evaporate(dur: number): void {
    this.eng.wash({ from: 1300, to: 200, dur: dur + 0.2, gain: 0.07, wet: 0.7 })
  }

  /** Phase 4 — λ and @ collapse into each other: a soft low absorb. */
  merge(dur: number): void {
    this.eng.tone({ freq: 220, endFreq: 73, dur: dur + 0.3, attack: 0.015, gain: 0.1, wet: 0.5 })
  }
  ...
  settle(dur: number): void {
    this.eng.tone({ freq: D4, dur: dur + 0.7, attack: dur * 0.35, gain: 0.055, type: 'triangle', wet: 0.8 })
    this.eng.tone({ freq: D4 * 1.5, dur: dur + 0.6, attack: dur * 0.45, gain: 0.035, wet: 0.8, delay: dur * 0.15 })
    this.eng.chime({ freq: D6, dur: 1.8, gain: 0.03, wet: 0.9, delay: dur * 0.55 })
  }
